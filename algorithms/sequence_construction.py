...
class IPF:
    ...
    def __init__(
        ...
        Args:
            ...
            tol: Convergence tolerance (max probability change). Default 1e-6.
        """
        self.prompt_manager = prompt_manager
    ...
    def _compute_constraints(self, secondary_classes: list, phi: int):
        ...
        if self.degree >= 2 and phi >= 2 and getattr(
            ...
            total_2 = sum(sup.values())
            if total_2 > 0:
                for (c1, c2), cnt in sup.items():
                    target = cnt / total_2
                    constraints.append(
                        (
                            target,
        ...
        if self.degree >= 3 and phi >= 3:
            ...
            for cp in self.prompt_manager.composite_prompts:
                ...
                for i in range(len(secs) - 2):
                    ...
                    total_3 += 1
            if total_3 > 0:
                for (c1, c2, c3), cnt in triple_count.items():
                    target = cnt / total_3
                    constraints.append(
                        (
                            target,
    ...
    def run(self, user_input: str, phi: int, large_k: int):
        ...
        if not outcomes:
            return []
        constraints = self._compute_constraints(secondary_classes, phi)
        d = self._ipf_iterate(outcomes, constraints)
        # Sort by probability descending, take top large_k
